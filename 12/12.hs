...
charLocations c xs = [(x,y) | (y,row) <- zip [0..] xs, x <- elemIndices c row]
...
bfs visited heightMap to (next:toVisit)
    ...
    | otherwise = bfs (IntSet.insert kNext visited) heightMap to (toVisit ++ newPoints) where
        kNext = k $ snd next
        newPoints = zip (repeat (1 + fst next)) $ filter possible $ map ($ snd next) [first (+1), first (subtract 1), second (+1), second (subtract 1)]
        possible (x, y) = (x >= 0) && (x < length (head heightMap)) && (y >= 0) && (y < length heightMap) && ((fromEnum (heightMap!!y!!x) - fromEnum (heightMap!!snd (snd next)!!fst (snd next))) <= 1) && IntSet.notMember (k (x,y)) visited
...
main = print . (solution1 &&& solution2) . lines =<< readFile "input"
